    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam) in enumerate(train_loader):
            ...
            if cfg.SOLVER.FP16_ENABLED:
                # Floating point 16 training
                with amp.autocast(enabled=True):
                    score, feat = model(img, target, cam_label=target_cam)  # cam_label không cần lắm
                    loss = loss_fn(score, feat, target, target_cam)         # target_cam cũng không cần
        ...
        end_time = time.time()
        time_per_batch = (end_time - start_time) / (len(train_loader) + 1)
            ...
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    torch.save(model.module.state_dict(), 
                               os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + '_{}.pth'.format(epoch)).replace("\\", "/"))
            else:
                torch.save(model.state_dict(),
                           os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + '_{}.pth'.format(epoch)).replace('\\', '/'))
        
        # Phần này không chạy vì val_loader sẽ là dataset.query và dataset.gallery
        # Mà dataset.query == dataset.train, dataset.gallery == dataset.train
        # => val_loader = 2 * train_loader
